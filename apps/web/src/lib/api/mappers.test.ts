...
describe('mapCompanyFromDb', () => {
  ...
  it('returns paymentMethod: null when payment_method is null', () => {
    const result = mapCompanyFromDb({ ...mockCompany, payment_method: null } as any);
    expect(result.paymentMethod).toBeNull();
  });

  it('returns paymentMethod: "bank_transfer" when payment_method is "bank_transfer"', () => {
    const result = mapCompanyFromDb({ ...mockCompany, payment_method: 'bank_transfer' } as any);
    expect(result.paymentMethod).toBe('bank_transfer');
  });

  it('returns paymentMethod: null for unknown/legacy values', () => {
    const result = mapCompanyFromDb({ ...mockCompany, payment_method: 'legacy_provider' } as any);
    expect(result.paymentMethod).toBeNull();
  });
});
