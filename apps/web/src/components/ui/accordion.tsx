...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between py-4 text-sm font-bold text-black transition-all hover:bg-neutral-50 px-2 text-left [&[data-state=open]>svg]:rotate-180",
        className
      )}
      {...props}
    >
