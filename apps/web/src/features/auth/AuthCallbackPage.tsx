    ...
    const handleCallback = async () => {
      try {
        ...
        if (!session) {
          ...
          if (code) {
            logger.warn(
              'No session after callback — likely different browser (PKCE verifier missing)'
            );
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-muted">
      <div className="w-full max-w-md">
        <SkeletonForm />
        <p className="mt-4 text-center text-muted-foreground">Completando autenticación...</p>
      </div>
    </div>
  );
};
