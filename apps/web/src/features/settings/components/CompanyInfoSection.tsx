...
const serviceLabelToValueMap: Map<string, QRDocumentType> = new Map(serviceOptions.map(o => [o.label, o.value]));
const serviceValueToLabelMap: Map<string, string> = new Map(serviceOptions.map(o => [o.value, o.label]));
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-base font-medium text-neutral-900">Informacion de la empresa</h2>
        </div>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Input id="companyPostalCode" label="Codigo Postal" name="postalCode" value={companyForm.postalCode || ''} onChange={handleCompanyFormChange} required error={companyFormErrors.postalCode} />
          </div>
          <Input id="companyAddress" label="Direccion" name="address" value={companyForm.address || ''} onChange={handleCompanyFormChange} required error={companyFormErrors.address} />
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Input id="companyCountry" label="Pais" name="country" value={companyForm.country || ''} onChange={handleCompanyFormChange} required error={companyFormErrors.country} />
          </div>
        </div>
        <div className="pt-2">
          <ChipGroup
            ...
            onChange={(selectedLabels) => {
              const newValues = selectedLabels.map(label => serviceLabelToValueMap.get(label)!) as QRDocumentType[];
