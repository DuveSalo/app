...
│   │   ├── notifications.ts
│   │   ├── subscription.ts # PayPal + MercadoPago lifecycle
...
├── paypal/
│   ├── PayPalProvider.tsx   # @paypal/react-paypal-js wrapper
│   ├── config.ts           # PayPal SDK script provider options
│   └── index.ts
├── mercadopago/
...
- **PayPal provider** wraps children — PayPal SDK handles PCI compliance.
