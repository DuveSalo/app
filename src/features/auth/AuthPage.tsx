...
const AuthPage: React.FC<{ mode: 'login' | 'register' }> = ({ mode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage === 'CONFIRMATION_NEEDED') {
    ...
    <AuthLayout variant="wizard" wizardSteps={wizardSteps} currentStep={1}>
        <div className="w-full max-w-md mx-auto">
            <div className="text-center mb-8">
                <h2 className="text-2xl font-bold text-gray-900">Cree su Cuenta</h2>
                <p className="text-gray-500 mt-1 text-sm">Comience gratis y centralice la gestión de su empresa.</p>
            </div>

            {showConfirmationMessage ? (
              <div className="bg-blue-50 border-l-4 border-blue-500 text-blue-900 p-6 rounded-md mb-6" role="alert">
                <div className="flex items-start">
                  <CheckIcon className="w-6 h-6 mr-3 flex-shrink-0 mt-0.5 text-blue-500" />
                  <div>
              ...
              <>
                ...
                <p className="text-gray-500 text-xs font-normal text-center mt-4 max-w-xs mx-auto">
