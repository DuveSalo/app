-- Fix: add is_admin() guard to admin_approve_payment and admin_reject_payment.
-- SECURITY DEFINER bypasses RLS, so the admin check must be explicit in the function body.
