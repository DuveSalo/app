-- Fix admin_approve_payment: cast v_period_end to date when updating subscription_renewal_date
...
BEGIN
  ...
  -- 2. Update company subscription status (cast text -> date)
  UPDATE companies
  ...
  VALUES (
    'approve_payment', p_admin_id, p_payment_id, 'manual_payment',
    jsonb_build_object(
      'company_id', v_company_id, 'plan_key', v_plan_key,
      'amount', p_amount, 'admin_email', p_admin_email
    )
  );
  ...
  VALUES (
    v_company_id, 'success', 'payment', 'Pago aprobado',
    'Tu transferencia bancaria fue aprobada. Tu suscripción está activa.',
    '/bank-transfer/status', 'manual_payments', p_payment_id, false
  );
END;
$$;
