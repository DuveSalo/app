-- ============================================================================
-- Clear trial state when a bank-transfer payment is approved.
-- ----------------------------------------------------------------------------
-- Root cause:
-- - A company can start on trial (companies.trial_ends_at set).
-- - If it pays before the trial ends, admin_approve_payment activates the
--   subscription but previously left trial_ends_at untouched.
-- - Admin details then showed both "fin del período de prueba" and renewal.
--
-- This forward migration fixes future approvals and repairs existing active
-- bank-transfer rows.
-- ============================================================================
...
WHERE trial_ends_at IS NOT NULL
  AND (is_subscribed = true OR subscription_status = 'active');

UPDATE subscriptions
SET next_billing_time = current_period_end,
    updated_at = NOW()
WHERE payment_provider = 'bank_transfer'
  AND status = 'active'
  AND next_billing_time IS NULL
