-- Update admin_approve_payment RPC to also insert a subscriptions row
-- when a bank transfer is approved, so the frontend can handle it uniformly
-- alongside MercadoPago subscriptions.
...
BEGIN
  ...
  -- Resolve plan key and name from company if not provided by caller
  ...
  -- Use provided plan name or derive a human-readable fallback
  ...
  -- 3. Insert a subscriptions row so the frontend handles bank transfers
  --    uniformly alongside MercadoPago subscriptions.
